...
Identifier = requests.post('http://172.22.22.10/BBINCO/TVS/Core/Controllers/PY.php', data=payload)
...
Pack = requests.post('http://172.22.22.10/BBINCO/TVS/Core/Controllers/Packages.php', data=payload)
...
def start(day, pos):
    ...
    Version = requests.post('http://172.22.22.10/BBINCO/TVS/Core/Controllers/PY.php', data=payload)
    Ver = json.loads(Version.content)
    Ver = Ver[0]
    
    payload = {'Option': 'GetOffsetZone'}
    Zone = requests.post('http://172.22.22.10/BBINCO/TVS/Core/Controllers/PY.php', data=payload)
    OffSetZone = json.loads(Zone.content)
    OffSetZone = OffSetZone[0]

    payload = {'Option': 'GetGatoTime'}
    GTime = requests.post('http://172.22.22.10/BBINCO/TVS/Core/Controllers/PY.php', data=payload)
    ...
    for Package in Packages:
        ...
        x = requests.post('http://172.22.22.10/BBINCO/TVS/Core/Controllers/PY.php', data=payload)
        ...
        with open('/var/www/html/BBINCO/TVS/Core/Controllers/Epg/'+IDF['IDF']+'/epg_'+day.strftime("%Y%m%d") + '_' + str(Package["id_paquete"]) + '.json', 'w', encoding='ascii') as file:
            json.dump(data, file, indent=4)

        with open('/var/www/html/BBINCO/TVS/Core/Controllers/Epg/'+IDF['IDF']+'/epg_'+day.strftime("%Y%m%d") + '_' + str(Package["id_paquete"]) + '.json', 'r') as file:
            filedata = file.read()

        filedata = filedata.replace('[', '').replace(']', '')
        with open('/var/www/html/BBINCO/TVS/Core/Controllers/Epg/'+IDF['IDF']+'/epg_'+day.strftime("%Y%m%d") + '_' + str(Package["id_paquete"]) + '.json', 'w') as file:
            file.write(filedata)
            print('/var/www/html/BBINCO/TVS/Core/Controllers/Epg/'+IDF['IDF']+'/epg_'+day.strftime("%Y%m%d") + '_' + str(Package["id_paquete"]) + '.json ', 'CREADO')
