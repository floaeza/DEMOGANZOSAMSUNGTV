...
function GetCurrentHour(){
    ...
    var GDate = new Date();
    var offsetHoras = offsetMegaGeneralSinTania;
        GDate.setHours(GDate.getHours() + offsetHoras);
...
function getInfoFromServer(option) { 
    ...
    switch (option) {
        ...
        break;
        case 'getVideosModuleRH':
            $.ajax({
                type: "POST",
                url: 'http://' + ServerIp +sourceController+'ModuleRH.php',
                data: { 
                    Option    : 'getVideosList',
                }, 
                async: false,
...
function newDigitalChannel(){
    ...
    var  videosList     = null;                       
    var  audiosList     = null;
        if(moduleToPlayVideos === 'promo'){
              videosList    = getInfoFromServer('getDigitalVideos');                        
              audiosList    = getInfoFromServer('getDigitalAudios');
              ...
              var  digitalMediaType   = getInfoFromServer('getDigitalMediaType'); 
                  ...
                  ShowInfo();
        }else if(moduleToPlayVideos === 'rh'){
                    videosList      = getInfoFromServer('getVideosModuleRH');                        
                    audiosList      = getInfoFromServer('getDigitalAudios');
               var  device             = getDevice();
                    playMultimedia(device, videosList, 0);
                    digitalMediaTypeToPlay = 'videos';
...
function getInfoToRH(){
    ...
    MenuDateRH.textContent = FormatDate;
    MenuHourRH.textContent = FormatHour;
    setTimeout(function(){
        getInfoToRH();
    }, 60000);
